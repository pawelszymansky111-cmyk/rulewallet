  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-10 lg:px-8">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
        ...
        <Card><CardHeader><CardTitle>Decision evidence</CardTitle><CardDescription>{decision.summary}</CardDescription></CardHeader><CardContent className="divide-y divide-border">{decision.rules.map((rule) => <div key={rule.id} className="flex items-start gap-3 py-3"><span className={`mt-0.5 grid size-5 place-items-center rounded-full ${rule.passed ? "bg-primary/10 text-primary" : "bg-red-400/10 text-red-300"}`}>{rule.passed ? <Check className="size-3.5" /> : <CircleX className="size-3.5" />}</span><div className="flex-1"><div className="flex justify-between gap-3 text-sm"><span>{rule.label}</span><span className={`font-mono text-[11px] ${rule.passed ? "text-muted-foreground" : "text-red-300"}`}>{rule.passed ? "PASS" : "FAIL"}</span></div><p className="mt-1 text-xs text-muted-foreground">{rule.detail}</p></div></div>)}</CardContent></Card>
      </div>

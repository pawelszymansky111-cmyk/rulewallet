      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            The same complete console in both modes—Simple explains each action,
            while Pro uses operator terminology.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button asChild variant="outline">
            <Link href="/app/services">
              <BookUser /> Trusted addresses
            </Link>
          </Button>
          <Button asChild variant="outline">
            <Link href="/app/agent">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.4fr_0.6fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Example receipts remain visible while live testnet requests appear
              in the command center above.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {requests.map((request) => (
                  <TableRow key={request.id}>
                    ...
                    <TableCell>
                      <DecisionBadge status={request.status} />
                    </TableCell>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>RH testnet trading agent</CardDescription>
          </CardHeader>
            ...
            <div className="rounded-lg border border-grid bg-background/45 p-4">
              ...
              <dl className="mt-4 space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-mono">$250</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Daily limit</dt>
                  <dd className="font-mono">$1,000</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Approval above</dt>
                  <dd className="font-mono">$100</dd>
                </div>
              </dl>
            </div>
            <Button asChild variant="outline" className="w-full">
              <Link href="/app/policies/pol_rh_trade_01">
                View policy <ArrowRight />
              </Link>
            </Button>
            <Button
              variant="destructive"
              className="w-full"
              disabled
              title="Persistence is disabled in the prototype"
            >
              <Pause /> Pause unavailable in demo
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mt-6 border-primary/15 bg-primary/[0.035]">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <WalletCards className="size-4 text-primary" /> Connect an agent
          </CardTitle>
          <CardDescription>
            Grant the onchain AGENT_ROLE to a separate testnet address. The
            agent can propose only policy-bound calls; it never receives the
            owner wallet seed or admin role.
          </CardDescription>
        </CardHeader>
      </Card>
    </main>
